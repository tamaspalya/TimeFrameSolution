...
namespace TimeFrameWebSite.Controllers
{
    public class AccountsController : Controller
    {
        ...
        public AccountsController(ITimeFrameApiClient client)
        {
            _client = client;
        }
        ...
        public async Task<IActionResult> Login([FromForm] TimeFrameWebAPIClient.DTOs.LoginDto loginInfo, [FromQuery] string returnUrl)
        {
            ...
            {
                return RedirectToAction("Index", "Dashboard"); // change for dashboard later
            }
            return View();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Register([FromForm] RegisterDto registerInfo, string confirm_password, [FromQuery] string returnUrl)
        {
            int userId = -1;
            if(registerInfo.Password != confirm_password)
            {
                ViewBag.ErrorMessage = "Passwords must match!";

            }
            else
            {
                userId = await _client.CreateUserAsync(registerInfo);
                return RedirectToActionPermanent("Login", "Accounts");
            }
            return View();
        }
